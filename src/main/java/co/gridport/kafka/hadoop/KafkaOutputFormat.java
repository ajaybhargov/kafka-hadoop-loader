...
public class KafkaOutputFormat<K, V> extends FileOutputFormat<K, V> {
     ...
     * @param job  the job conf for the job
     * @param name the leaf file name for the output file (such as part-00000")
     * @param arg3 a progressable for reporting progress.
    ...
    public RecordWriter<K, V> getRecordWriter(TaskAttemptContext context) throws IOException {
        ...
        return new RecordWriter<K, V>() {
            ...
            public void write(K key, V value) throws IOException {
                ...
                try {
                    if (rw == null) {
                        Path file = new Path(((FileOutputCommitter) getOutputCommitter(taskContext)).getWorkPath(),
                            getUniqueFile(taskContext, keyBasedPath + "/" + taskContext.getJobID().toString().replace("job_", ""),
                                extension));

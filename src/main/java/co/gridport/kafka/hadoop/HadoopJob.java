...
public class HadoopJob extends Configured implements Tool {
    ...
    private static org.slf4j.Logger log = LoggerFactory.getLogger(HadoopJob.class);
    ...
    public int run(String[] args) throws Exception {
        ...
        if (cmd.hasOption("topics")) {
            ...
            System.out.println("Using topics: " + conf.get("kafka.topics"));
        } else if (cmd.hasOption("filter")) {
            conf.set("kafka.topic.filter", cmd.getOptionValue("filter"));
            System.out.println("Using topic filter: " + conf.get("kafka.topic.filter"));
        ...
        System.out.println("Registering under consumer group: " + conf.get("kafka.groupid"));

        conf.set("kafka.zk.connect", cmd.getOptionValue("zk-connect", "hq-mharis-d02:2181"));

        System.out.println("Using ZooKepper connection: " + conf.get("kafka.zk.connect"));

        if (cmd.getOptionValue("autooffset-reset") != null) {
            conf.set("kafka.watermark.reset", cmd.getOptionValue("autooffset-reset"));
            System.out.println("SHOULD RESET OFFSET TO: " + conf.get("kafka.watermark.reset"));
        }
        ...
        System.out.println("EXPECTING MESSAGE FORMAT: " + conf.get("input.format"));

        JobConf jobConf = new JobConf(conf);
        if (cmd.hasOption("remote")) {
            String ip = cmd.getOptionValue("remote");
            System.out.println("Default file system: hdfs://" + ip + ":8020/");
            jobConf.set("fs.defaultFS", "hdfs://" + ip + ":8020/");
            System.out.println("Remote jobtracker: " + ip + ":8021");
        ...
        if (new File(jarTarget.toUri()).exists()) {
            ...
            System.out.println("Using target jar: " + jarTarget.toString());
        } else {
            //running from jar remotely or locally
            jobConf.setJarByClass(getClass());
            System.out.println("Using parent jar: " + jobConf.getJar());
        }
        ...
        System.out.println("Output hdfs location: " + hdfsPath);
        System.out.println("Output hdfs compression: " + KafkaOutputFormat.getCompressOutput(job));
    ...
    private Options buildOptions() {
        ...
        options.addOption(
            OptionBuilder.withArgName("groupid").withLongOpt("consumer-group").hasArg().withDescription("kafka consumer groupid")
                .create("g"));
        options.addOption(
            OptionBuilder.withArgName("zk").withLongOpt("zk-connect").hasArg().withDescription("ZooKeeper connection String").create("z"));

        options.addOption(OptionBuilder.withArgName("offset").withLongOpt("offset-reset").hasArg()
            .withDescription("Reset offset to start or end of the stream e.g. 'earliest' or 'latest'").create("o"));
        ...
        options.addOption(
            OptionBuilder.withArgName("ip_address").withLongOpt("remote").hasArg().withDescription("Running on a remote hadoop node")
                .create("r"));

        options.addOption(OptionBuilder.withLongOpt("help").withDescription("Show this help").create("h"));

        options.addOption(
            OptionBuilder.withArgName("topic_filter").withLongOpt("filter").hasArg().withDescription("Topic filter").create("f"));
